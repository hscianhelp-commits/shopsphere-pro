// Social share with deep link → fallback for native app opening on mobile

export type SocialPlatform = 'facebook' | 'whatsapp' | 'twitter' | 'linkedin' | 'instagram' | 'tiktok' | 'telegram';
...
export function shareOnPlatform(platform: SocialPlatform, text: string, url: string) {
  const encodedText = encodeURIComponent(text + ' ' + url);
  const encodedUrl = encodeURIComponent(url);

  const configs: Record<SocialPlatform, ShareConfig> = {
    facebook: {
    ...
    instagram: {
      deep: `instagram://sharesheet?text=${encodedText}`,
    ...
    telegram: {
      deep: `tg://msg_url?url=${encodedUrl}&text=${encodeURIComponent(text)}`,
      fallback: `https://t.me/share/url?url=${encodedUrl}&text=${encodeURIComponent(text)}`,
    },
  };
...
export const socialIcons: Record<SocialPlatform, { svg: string; color: string; viewBox?: string; size?: number }> = {
  facebook: {
    color: '#1877F2',
...
// Render a social share button
export const allPlatforms: SocialPlatform[] = ['facebook', 'whatsapp', 'telegram', 'instagram', 'tiktok', 'twitter', 'linkedin'];
